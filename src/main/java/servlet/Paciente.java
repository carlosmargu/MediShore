...
@WebServlet(
        ...
        urlPatterns = {"/verDisponibilidad", "/reservar", "/paciente", "/contrato", "/vercontrato",
            "/contratocodigo","/vercitas","/prueba","/vercitast"})
...
public class Paciente extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (url.equalsIgnoreCase("/verDisponibilidad")) {
            ...
            //out.write(facade.AgregarCita(hora, fecha, hora, paciente).getBytes());
        ...
        else if (url.equalsIgnoreCase("/vercitas")) {
            String fecha = req.getParameter("fecha");
            facade.getCitas(fecha);
            ServletOutputStream out = resp.getOutputStream();
            out.write(facade.getCitas(fecha).getBytes());
            out.flush();
            out.close();
        }
        else if (url.equalsIgnoreCase("/vercitast")) {
            ServletOutputStream out = resp.getOutputStream();
            out.write(facade.vercitasjj().getBytes());
            out.flush();
            out.close();
        }
